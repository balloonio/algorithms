class Solution:
    def orderOfLargestPlusSign(self, N, mines):
        ...
        for i in range(N):
            for j in range(N):
                up[i][j] = (
                    0 if (i, j) in mine_set else (1 if i - 1 < 0 else up[i - 1][j] + 1)
                )
                lf[i][j] = (
                    0 if (i, j) in mine_set else (1 if j - 1 < 0 else lf[i][j - 1] + 1)
                )

        for i in reversed(range(N)):
            for j in reversed(range(N)):
                dn[i][j] = (
                    0 if (i, j) in mine_set else (1 if i + 1 >= N else dn[i + 1][j] + 1)
                )
                rt[i][j] = (
                    0 if (i, j) in mine_set else (1 if j + 1 >= N else rt[i][j + 1] + 1)
                )
        result = 0
        for i in range(N):
            for j in range(N):
